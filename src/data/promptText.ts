...
export const accessiblePromptText = `Build an accessible React carousel component in ONE file, plain JSX, styled with Tailwind. No external UI libraries.
...
Requirements
- Component and interactions MUST meet WCAG 2.2 Level AA accessibility guidelines
- Render slides horizontally in a track that translates by -index * 100%.
- Each slide uses <img src={slide.image} alt={slide.description} className="block w-full h-auto" />.
- Provide Previous and Next buttons.
- Provide dot navigation below the viewport.
